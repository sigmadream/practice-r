...
### 범주형 변수 처리 (Turn categoricals into booleans)
...
모든 값이 0으로 채워진 새로운 datafream을 생성하되, 각 category는 개별 컬럼을 만들어 줍니다.
...
### 수치형 변수 처리(Scale the numerical variables)

수치의 단위(unit)이 일정하지 않기 때문에 수치형 변수를 일괄로 처리하도록 하겠습니다. 여기서는 예측 변수(y)인 `median_house_value`를 제외한 모든 수치 데이터를 조정합니다.
...
위 결과에서 평균 제곱 오차(Mean Squared Error, MSE)가 포함된 `%IncMSE`는 변수 중요도의 척도입니다. 주어진 변수가 섞일 때, *예측의 평균 제곱 오차(MSE)* 의 증가를 측정하여 모델의 변수 중요성에 대한 지표로 사용됩니다. 따라서 더 높은 숫자는 더 중요한 예측 변수입니다.
...
분석을 통해 소득 중앙값(`median_income`)이 주택 가격(`median_house_value`)에 가장 큰 영향을 미친다고 판단할 수 있으며, RMSE를 통해서 알 수 있듯이 랜덤 포레스트의 예측값이 더 우수하다.
...
XGBoost를 사용하면 RMSE 값을 좀 더 줄 일 수 있다.

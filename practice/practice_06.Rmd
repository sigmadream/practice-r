...
knitr::opts_chunk$set(echo=F, message=F, warning=F, fig.height = 8, cache=T, dpi = 300, dev = "png")
library(tidyverse)
library(hflights)
```
...
```{r}
str(mpg)
mpg %>% select(displ, year, cyl, cty, hwy)
...
## map over more than one argument
...
draw multiple plots per one tibble with map() and export plots into separate .png files

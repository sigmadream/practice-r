...
str_trunc(string = fruit, width = 5, side = "left", ellipsis = "...") %>% head()   # left side truncating
str_trunc(string = fruit, width = 5, side = "right", ellipsis = "...") %>% head()  # right side truncating
str_trunc(string = fruit, width = 5, side = "center", ellipsis = "...") %>% head() # center side truncating
```
...
mtcars %>% head()
str_glue_data(mtcars, "The car {rownames(mtcars)}: {hp} horsepower, {cyl} number of cylinders and consumption {mpg} miles per gallon") %>% head()
```
...
str_subset(words, "(..)\\1") %>% head()   # \1 is a group reference 1st group, double backslash ~ escaping
